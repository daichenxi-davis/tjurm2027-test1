...
void my_strcat(char *str_1, char *str_2) {
     ...
     */
    int len,p;
    len=p=my_strlen(str_1);
    str_1[len]=0;
    while(str_2[p-len]!='\0') str_1[p]=str_2[p-len],p++;
    str_2[p-len]='\0';
    // IMPLEMENT YOUR CODE HERE
}
...
char* my_strstr(char *s, char *p) {
    ...
    for(int i=0;i<len1-len2;i++)
    {
        bool ret=false;
        int pos=i;
        while(s[pos]==p[pos-i]&&p[pos-i]!='\0') pos++;
        if(p[pos-i]=='\0') return &s[i];
    }
    // IMPLEMENT YOUR CODE HERE
    return nullptr;
}
...
void rgb2gray(float *in, float *out, int h, int w) {
    ...
    for(int i=0;i<h;i++)
        for(int j=0;j<w;j++)
        {
            ...
            float g=*pos_in++;
            float b=*pos_in++;
            *pos_out=0.1140*b+0.5870*g+0.2989*r;
            pos_out++;
        }
    // IMPLEMENT YOUR CODE HERE
    // ...
...
void resize(float *in, float *out, int h, int w, int c, float scale) {
    ...
    auto swap=[&](int &a,int &b){a^=b^=a^=b;};
    // swap(new_h,new_w);
    // swap(h,w);
    auto max=[&](int a,int b){return a>b?a:b;};
    auto min=[&](int a,int b){return a<b?a:b;};
    auto get=[&](int x,int y,int col)
    {
        // printf("(%d,%d,%d)\n",x,y,c);
        return in[(x*w+y)*c+col];
    };
    int s=0;
    auto chk=[&](int &a,int b){a=min(a,b);};
    float *pos=out;
    for(int i=0;i<new_h;i++)
        for(int j=0;j<new_w;j++)
        {
            float x0=i/scale,y0=j/scale;
            // if(x0>h-1) x0=h-1;
            // if(y0>w-1) y0=w-1;
            // printf("(%f,%f)\n",x0,y0);
            int x1=x0,y1=y0;
            int x2=x1+1,y2=y1+1;
            chk(x1,h-1);chk(x2,h-1);chk(y1,w-1);chk(y2,w-1);
            // printf("(%d,%d,%d,%d)\n",x1,x2,y1,y2);
            float dx=x0-x1,dy=y0-y1;
            for(int k=0;k<c;k++)
            {
                float p1=get(x1,y1,k);
                float p2=get(x1,y2,k);
                float p3=get(x2,y1,k);
                float p4=get(x2,y2,k);
                *pos=p1*(1-dx)*(1-dy)+p2*(1-dx)*dy+p3*dx*(1-dy)+p4*dx*dy;
                pos++;
                // printf("%d\n",s++);
            }
/*
     ...
     *                      Dx * Dy            Dx * Dy
     * */
            // int Dx=x2-x1,Dy=y2-y1,dx=x0-x1,dy=y0-y1;
        }
    // IMPLEMENT YOUR CODE HERE
